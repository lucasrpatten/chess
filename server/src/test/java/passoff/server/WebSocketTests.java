...
public class WebSocketTests {
    ...
    public void makeMoveBadAuthtoken() {
        ...
        makeMove(new WebsocketUser(white.username(), "badAuth"), gameID, move, false, false, Set.of(black, observer),
                Set.of());
    }
    ...
    public void leaveGame() {
        ...
        // have white player leave--all other players get notified, white player should
        // not be
    ...
    public void multipleConcurrentGames() {
        ...
        // player leave in first game - only users remaining in first game should be
        // notified
    ...
    private void assertHttpOk(TestResult result, String context) {
        Assertions.assertEquals(200, serverFacade.getStatusCode(),
                String.format("HTTP Status code was not 200 for %s, was %d. Message: %s", context,
                        serverFacade.getStatusCode(), result.getMessage()));
    }

    private void connectToGame(WebsocketUser sender, int gameID, boolean expectSuccess, Set<WebsocketUser> inGame,
    ...
    private void makeMove(WebsocketUser sender, int gameID, ChessMove move, boolean expectSuccess,
        ...
        if (extraNotification && actualMessages.get(sender.username()).size() > 1) {
            assertCommandMessages(actualMessages, expectSuccess, sender, types(LOAD_GAME, NOTIFICATION), inGame,
                    types(LOAD_GAME, NOTIFICATION, NOTIFICATION), otherClients);
        }
        else {
            assertCommandMessages(actualMessages, expectSuccess, sender, types(LOAD_GAME), inGame,
                    types(LOAD_GAME, NOTIFICATION), otherClients);
        }
    }

    private void resign(WebsocketUser sender, int gameID, boolean expectSuccess, Set<WebsocketUser> inGame,
            Set<WebsocketUser> otherClients) {
        ...
        assertCommandMessages(actualMessages, expectSuccess, sender, types(NOTIFICATION), inGame, types(NOTIFICATION),
                otherClients);
    }
    ...
    private Map<String, Integer> expectedMessages(WebsocketUser sender, int senderExpected, Set<WebsocketUser> inGame,
            int inGameExpected, Set<WebsocketUser> otherClients) {
        Map<String, Integer> expectedMessages = new HashMap<>();
        expectedMessages.put(sender.username(), senderExpected);
        expectedMessages
                .putAll(inGame.stream().collect(Collectors.toMap(WebsocketUser::username, s -> inGameExpected)));
    ...
    private void assertCommandMessages(Map<String, List<TestMessage>> messages, boolean expectSuccess,
            WebsocketUser user, ServerMessage.ServerMessageType[] userExpectedTypes, Set<WebsocketUser> inGame,
        ...
        for (WebsocketUser otherUser : otherClients) {
            assertMessages(otherUser.username(), new ServerMessage.ServerMessageType[0],
                    messages.get(otherUser.username()));
        }
    }
    ...
    private void assertLoadGame(String username, TestMessage message) {
        ...
        Assertions.assertNotNull(message.getGame(),
                "%s's LOAD_GAME message did not contain a game (Make sure it's specifically called 'game')"
                        .formatted(username));
    ...
    private void assertNotification(String username, TestMessage message) {
        ...
        Assertions.assertNull(message.getErrorMessage(), "%s's NOTIFICATION message contained an error message: %s"
                .formatted(username, message.getErrorMessage()));
    }
    ...
    private void assertError(String username, TestMessage message) {
        ...
        Assertions.assertNotNull(message.getErrorMessage(),
                "%s's ERROR message did not contain an error message (Make sure it's specifically called 'errorMessage')"
                        .formatted(username));
        Assertions.assertNull(message.getGame(),
